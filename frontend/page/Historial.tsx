...
export default function Historial() {
  ...
  const porcentajeCumplimiento = useMemo(() => {
    const dosisGestionadas = dosis.filter((item) => {
      const estado = item.estado?.toLowerCase();
      return estado === "tomado" || estado === "omitido";
    });

    if (!dosisGestionadas.length) {
      return 0;
    }

    const tomadas = dosisGestionadas.filter((item) => item.estado?.toLowerCase() === "tomado").length;
    return Math.round((tomadas * 100) / dosisGestionadas.length);
  }, [dosis]);
